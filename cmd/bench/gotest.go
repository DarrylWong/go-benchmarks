...
// baseline commit is checked out in baselineDir, and experiment commit is
...
func goTestSubrepo(tc *toolchain, subRepo, baselineDir, experimentDir string) error {
	...
	case "tools":
		...
		for _, test := range tests {
			...
			err = tc.Do(benchmarkDir, "test", "-v", "-short", "-bench=.", fmt.Sprintf(`-gopls_path="%s"`, goplsPath), "-count=5")
			if err != nil {
