...
func (c *config) diagnosticDataPath(typ diagnostics.Type, prefix string) string {
	var fname string
	switch typ {
	...
	return filepath.Join(c.tmpDir, prefix+fname)
}
...
func reportFromBenchmarkOutput(b *driver.B, output string) error {
...
func getQuantileLatency(quantile, output string) (float64, error) {
	...
	return strconv.ParseFloat(re.FindStringSubmatch(output)[re.SubexpIndex("value")], 64)
}
		...
		if driver.DiagnosticEnabled(diagnostics.CPUProfile) {
			for i, inst := range instances {
				finishers = append(finishers, server.PollDiagnostic(
					...
					cfg.diagnosticDataPath(diagnostics.CPUProfile, strconv.Itoa(i)+"-"),
		...
		if driver.DiagnosticEnabled(diagnostics.Trace) {
			...
			for i, inst := range instances {
				stopTrace := server.PollDiagnostic(
					inst.host(clientPort),
					cfg.diagnosticDataPath(diagnostics.Trace, strconv.Itoa(i)+"-"),
					cfg.bench.reportName,
					diagnostics.Trace,
				)
			...
			for i, inst := range instances {
				...
				finishers = append(finishers, func() uint64 {
					n, err := server.CollectDiagnostic(
						inst.host(clientPort),
						cfg.diagnosticDataPath(diagnostics.MemProfile, strconv.Itoa(i)+"-"),
						cfg.bench.reportName,
