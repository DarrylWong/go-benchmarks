...
type cockroachdbInstance struct {
	name     string
	sqlPort  int
	httpPort int // Not used in this benchmark, but will cause port collisions if not specified.
	cmd      *exec.Cmd
	output   bytes.Buffer
}
...
func runBenchmark(b *driver.B, cfg *config, instances []*cockroachdbInstance) (err error) {
	...
	case <-time.After(cfg.bench.timeout):
		if err := cmd.Process.Kill(); err != nil {
			return errors.Wrap(err, "error killing benchmark process, benchmark timed out")
		}
